...
const BackgroundColor: React.FC<BackgroundColorProps> = ({
  ...
  const applyBackgroundColor = (color: string) => {
    if (editorRef.current) {
      ...
      applyBackgroundColorToRange(range, color);
  ...
  const applyBackgroundColorToSingleRange = (range: Range, color: string) => {
    ...
    if (startNode.nodeType === Node.TEXT_NODE) {
      ...
      const isBgSpan =
        ...
        (parentNode.style.backgroundColor || parentNode.classList.contains('default-bg-color'));
      ...
      if (isBgSpan) {
        if (isFullNodeSelected) {
          ...
          if (color === 'transparent') {
            ...
            parentNode.className = 'default-bg-color';
          } else {
            parentNode.style.backgroundColor = color;
            parentNode.classList.remove('default-bg-color');
          }
        } else {
          ...
          beforeSpan.style.backgroundColor = parentNode.style.backgroundColor;
          beforeSpan.textContent = beforeText;

          const selectedSpan = document.createElement('span');
          if (color === 'transparent') {
            selectedSpan.style.backgroundColor = '';
            selectedSpan.className = 'default-bg-color';
          } else {
          ...
          afterSpan.style.backgroundColor = parentNode.style.backgroundColor;
        ...
        if (color === 'transparent') {
          newBgSpan.style.backgroundColor = '';
          newBgSpan.className = 'default-bg-color';
        } else {
          newBgSpan.style.backgroundColor = color;
        }

        newBgSpan.textContent = textContent.substring(startOffset, endOffset);
        ...
        if (parent) {
          ...
          parent.insertBefore(newBgSpan, startNode);
          ...
          if (selection) {
            selection.removeAllRanges();
            const newRange = document.createRange();
            newRange.setStartAfter(newBgSpan);
            newRange.setEndAfter(newBgSpan);
            selection.addRange(newRange);
          }
        }
      }
      ...
      if (color === 'transparent') {
        ...
        newSpan.className = 'default-bg-color';
      } else {
  ...
  const applyBackgroundColorToRange = (range: Range, color: string) => {
    const startNode = range.startContainer;
    ...
    if (startNode !== endNode) {
      ...
      for (const textNode of textNodesInRange) {
        ...
        if (color === 'transparent') {
          ...
          span.className = 'default-bg-color';
        } else {
    ...
    applyBackgroundColorToSingleRange(range, color);
  };


  const shouldShowBorder = selectedColor === 'transparent' || selectedColor === '#FFFFFF';
    ...
    <div className="background-color-container">
      ...
      >
        <span className="button-icon">
          <Icon width={28} height={30} className='Bold' />
        </span>

        {/* <span
          className={`color-preview ${shouldShowBorder ? 'with-border' : ''}`}
          style={{ backgroundColor: selectedColor === 'transparent' ? 'white' : selectedColor }}
        >
        </span> */}
      </button>
      ...
      {isOpen && (
        <div className="color-picker-dropdown" ref={colorPickerRef} role="dialog" aria-label="Color picker">
          {!showCustomPicker ? (
            <>
              <div className="color-option automatic" onClick={handleAutomatic} role="button" tabIndex={0} aria-label="Automatic (no background color)">
              ...
              <div className="more-colors-option" onClick={showMoreColors} role="button" tabIndex={0} aria-label="Show more colors">
            ...
            <div className="custom-color-picker">
              <HexColorPicker color={selectedColor === 'transparent' ? '#FFFFFF' : selectedColor} onChange={handleCustomColorSelect} />
              <button
